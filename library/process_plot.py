...
class Plot:
    ...
    def boxplot1(self, df: pd.DataFrame):
        ...
        for col in numeric_columns:
            ...
            g.savefig(
                os.path.join(self.output_dir, col + "_species_boxplot" + ".pdf"),
                transparent=True,
                bbox_inches="tight",
            )
            g.savefig(
                os.path.join(
                    self.output_dir, "graph_previews", col + "_species_boxplot" + ".png"
                ),
                transparent=True,
                bbox_inches="tight",
            )
    ...
    def boxplot2(self, df: pd.DataFrame):
        ...
        for col in numeric_columns:
            ...
            for ax in g.axes.flat:
                ...
                ax.set_ylabel(ax.get_ylabel(), fontsize=12)
            g.savefig(
                os.path.join(self.output_dir, col + "_sex_paired_boxplot" + ".pdf"),
                transparent=True,
                bbox_inches="tight",
            )
            g.savefig(
    ...
    def pcaplot(self, df: pd.DataFrame):
        ...
        if n_components < 2:
            warnings.warn(
                "Less than 2 PCA components have been calculated.\nPCA plot is not possible"
            )
            return
        ...
        for c1, c2 in plot_pairs:
            ...
            for target, color, marker in zip(targets, colors, markers):
                ...
                ax.scatter(
                    df.loc[indicesToKeep, f"PC{c1}"],
                    df.loc[indicesToKeep, f"PC{c2}"],
                    color=color,
                    marker=marker,
                    s=50,
                    label=target,
            ...
            fig.savefig(
                os.path.join(self.output_dir, f"pca{c1}{c2}_plot" + ".pdf"),
                transparent=True,
                bbox_inches="tight",
            )
            fig.savefig(
                os.path.join(
                    self.output_dir, "graph_previews", f"pca{c1}{c2}_plot" + ".png"
                ),
                transparent=True,
                bbox_inches="tight",
            )
    ...
    def ldaplot(self, df: pd.DataFrame):
        ...
        fig.savefig(
            os.path.join(self.output_dir, f"{label}_lda_plot" + ".pdf"),
            transparent=True,
            bbox_inches="tight",
        )
        fig.savefig(
            os.path.join(
                self.output_dir, "graph_previews", f"{label}_lda_plot" + ".png"
            ),
            transparent=True,
            bbox_inches="tight",
        )
